...
<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab>
    ```bash
    pnpm install -D @mfauzanap_pandacss/dev
    pnpm panda init --postcss
    ```
  </Tab>
  <Tab>
    ```bash
    npm install -D @mfauzanap_pandacss/dev
    npx panda init --postcss
    ```
  </Tab>
  <Tab>
    ```bash
    yarn add -D @mfauzanap_pandacss/dev
    yarn panda init --postcss
    ```
  </Tab>
  <Tab>
    ```bash
    bun add -D @mfauzanap_pandacss/dev
...
If you are having issues with the PostCSS plugin similar to `Cannot find module '@mfauzanap_pandacss/dev/postcss'`,
